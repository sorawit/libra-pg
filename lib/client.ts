...
export class LibraClient {
  ...
  public async transferCoins(
    ...
    const tx = LibraTransaction.createTransfer(receipientAddress, new BigNumber(numCoins));
    // console.log(tx);
    const response = await this.execute(tx, sender);
    return response;
  }
  ...
  public async execute(transaction: LibraTransaction, sender: Account): Promise<SubmitTransactionResponse> {
    ...
    if (sequenceNumber.isNegative()) {
      ...
      console.log('SEQ', sequenceNumber.toString());
    }
    ...
    arg1.setData(transaction.program.arguments[0].value);

    const arg2 = new TransactionArgument();
    arg2.setType(TransactionArgument.ArgType.U64);
    arg2.setData(transaction.program.arguments[1].value);
    ...
    console.log(transaction.expirationTime.toNumber());
    ...
    console.log(sha3.digest());

    signedTransaction.setSenderSignature(sender.keyPair.sign(sha3.digest()));
    // console.log(rawTransaction.serializeBinary().toString());
